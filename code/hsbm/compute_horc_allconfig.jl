...
function load_hypergraph_edges(rows)
    ...
    for (row_idx, row) in enumerate(rows)
        ...
        num_comms_raw = get_with_fallback(row, :num_comms)
        if hg_raw === nothing || members_raw === nothing || num_comms_raw === nothing
            continue
        end
        if ismissing(hg_raw) || ismissing(members_raw) || ismissing(num_comms_raw)
            continue
        end
        num_comms = num_comms_raw isa Integer ? Int(num_comms_raw) : parse(Int, String(num_comms_raw))
        num_comms == 4 || continue
...
function resolve_paths(args::Vector{String})
    ...
    default_output = joinpath(data_dir, "nu_hsbm_horc.csv")
